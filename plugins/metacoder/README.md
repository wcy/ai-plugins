...
- **Subagents (fan-out).** `mreverse` deep-dive readers + cross-repo inconsistency readers; `mverify` per-module/per-repo conformance shards; `mspec`'s up-front risk scan; `mplan`'s post-generation story validators.
