...
2. **Ask for the mode and the requirements gate in one call.** Run `python3 ${CLAUDE_PLUGIN_ROOT}/tools/mc.py spec mode <target>`: it reports the CREATE/UPDATE mode and the target's own requirements-gate result independently, in a single call. A failed invocation is a **hard error** — halt the phase and report it; there is no prose fallback, and neither answer is ever re-derived by hand. **If the reported mode is CREATE and the requirements gate does not pass, run `mreq`'s BRAINSTORM Phase 1 first** (product/business-altitude clarify only — the need, who has it, why now, what success looks like; never a technical decision), folded into this same gate, not a second one; otherwise go straight to step 3. This is the one check `mquick` performs itself; every other decision — including the blast-radius test that separates a significant from a contained breaking change — stays delegated to the skill it's sequencing (`mexecute` performs that test at Phase E; `mquick` only relays the result, it doesn't decide it), and every other mechanical step reaches the tool through that skill rather than from here.
