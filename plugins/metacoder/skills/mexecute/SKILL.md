...
- **Story agent** (worktree-isolated). Its prompt names exactly one story and carries the three names above. It: (a) **creates its own worktree** with `git worktree add` inside `repos/<repo>/`, on the `story` branch cut from that repo's `integration` branch (the merged tip of the previous wave), at `worktree_path`; (b) loads **only** its `PLAN-*.md`, that story's Context Files, and the repo `CATALOG.yaml` — nothing else; (c) implements the story in `repos/<repo>/` (its single target repo) **task by task, running the story's `validation.increments` steps as it goes** — each step immediately after the Implementation Task its `task` index names (`1` is the first task), which is where `mc.py plan story-emit` already interleaved it into the story file; (d) **fixes a non-zero increment where it fired, before the next task begins** — never carrying a red increment forward; (e) runs the story's **Post-Story Validation** (`validation.post_story`) from `plan.yaml` (`kind: exit-code` commands must exit 0; `prose` steps it interprets), which remains the story's **merge gate** and stops being the first execution of its code; (f) returns the `story-report.schema.json` shape — status, branch, worktree path, files changed, validation result, any `breaking_contract_change`, any `contract_revisions` it built against, and any `spec_defect`. `agent()`'s `isolation: 'worktree'` option is **unparameterized** — it selects neither repo nor branch — so it is **not** the mechanism of story isolation; real `git worktree` commands, run by the story agent itself, are.
...
**If the `Workflow` tool is unavailable** in the environment, emulate the same structure by hand: for each wave in order, dispatch the story agents concurrently (`Agent` calls, one message — each story agent creates its own worktree with `git worktree add`, runs its `validation.increments` as it implements and its `validation.post_story` when the story is done), then run the **retry loop to exhaustion**, then the barrier/merge yourself, then run that wave's `validation` steps against the merged integration branch — the same ordering the Workflow path uses: after the merge, before you cut the next wave — then state persistence, then advance; on the slice's last wave, run Slice Acceptance and record it. The story-agent contract is the same one described above and below, dispatch mechanism notwithstanding: increments during implementation, each fixed before the next task begins, `post_story` as the merge gate. What matters is the model — waves in order, worktree-per-story, checks during each story and against the merged branch after each barrier, retry 3 per run, slice acceptance, sweep — not the tool. The `mc.py` calls are identical either way.

### The Story Agent Checks As It Goes

**Checking is continuous, not terminal.** A story's `validation.increments` steps run *during* implementation — each immediately after the Implementation Task its `task` index names — not collected up and run once the story is finished. They are the earliest point at which the story's code is executed, and they are what lets a failure name the increment that introduced it rather than the story it surfaced in.

**A non-zero increment is fixed where it fired, before the next task begins.** That is the whole rule, and it is the reason the increments exist. Carrying a red increment forward is what makes the eventual failure unattributable: three tasks later the story agent is debugging its own accumulated work rather than one change, and the report names a broken story instead of a broken increment. Fix it, re-run that step until it exits 0, and only then start the next task.

`validation.post_story` is unchanged in role: still the story's **merge gate**, still what sets `status: applied|failed` on the `story-report`, still what the barrier's Good/Salvageable judgment reads. What changed is that it is no longer the first execution of the story's code.

**This introduces no halt condition and spends no retry budget.** An increment that fails and is fixed is ordinary work inside one attempt — not a wave outcome, not a story outcome, not an attempt of its own. The halt conditions stay at the three listed below, and `RETRY_MAX = 3` attempts per story per run is untouched: an increment the agent cannot fix simply arrives at `post_story` red, and the existing retry loop answers it exactly as it always has.

A story carrying no `validation.increments` — every story of a graph below version 4 — is implemented and then post-story validated, exactly as before.
...
**An increment is not an attempt.** The unit the budget counts is the story attempt — one worktree, one branch, one dispatch. A story agent fixing a red `validation.increments` step mid-implementation is working *within* its current attempt and spends none of it, however many increments it fixes; `RETRY_MAX` and `retries` mean exactly what they meant before increments existed.
