...
2. **Resume point.** Read the plan's `state.yaml`. Stories already `applied` are **skipped**; the run resumes at the **first wave with any unfinished story**. A story still `failed` after prior retry exhaustion is retried **afresh** on this run (attempts reset for the new run, but its prior attempts stay recorded).
...
export const meta = {
  ...
  description: 'Ship plan <plan-id>: worktree-isolated waves, barrier merge, retry 3, conformance sweep',
...
for (const wave of WAVES) {
  ...
  // Fan out: one worktree-isolated agent per story, full wave concurrency.
  let results = await parallel(pending.map(story => () =>
    agent(storyPrompt(story, /*attempt*/ 1), {
      label: story, phase: `Wave ${wave.wave}`,
      isolation: 'worktree', schema: STORY_REPORT_SCHEMA,
    })))

  // Bounded auto-retry: up to RETRY_MAX attempts per failed story.
  for (let attempt = 2; attempt <= RETRY_MAX; attempt++) {
    ...
    const retried = await parallel(failed.map(r => () =>
      agent(storyPrompt(storyOf(r), attempt), {
        label: `${storyOf(r)}/${attempt}`, phase: `Wave ${wave.wave}`,
        isolation: 'worktree', schema: STORY_REPORT_SCHEMA,
      })))
...
phase('Sweep')
const sweep = await agent(mverifySweepPrompt(), { label: 'mverify-sweep', phase: 'Sweep' })
return { done: true, sweep }
```

In the skeleton, `STORY_REPORT_SCHEMA` is the parsed `${CLAUDE_PLUGIN_ROOT}/schemas/story-report.schema.json`; `BARRIER_SCHEMA` is a small inline shape you define for the barrier agent's return (`{ halt: bool, reason, merged: [...], deferred_breaks: [...] }`); and `storyPrompt`/`barrierPrompt`/`stateSays`/`storyOf`/`mergeAttempts` are illustrative helpers you build from `plan.yaml` + `state.yaml`. Adapt freely — the **contract** is: waves in order, one worktree-isolated agent per story at full concurrency, a barrier that validates/merges/persists, bounded retry 3, then the sweep.
...
- **mverify sweep agent** (step 6): invokes `/mverify` across every repo the plan touched.

**If the `Workflow` tool is unavailable** in the environment, emulate the same structure by hand: for each wave in order, dispatch the story agents concurrently (`Agent` calls with `isolation: 'worktree'` in one message), then run the barrier/merge + state persistence yourself, then the retry loop, then advance. The model (waves in order, worktree-per-story, barrier discretion, retry 3, sweep) is what matters — the `Workflow` tool is just the cleanest way to express it.
...
Up to **3 attempts per failed story** (`RETRY_MAX = 3`). Each attempt is a fresh worktree/branch (`…/<attempt>`), recorded in `state.yaml`. On exhaustion the story is left `failed` and the run **halts and reports** — it never advances a wave on red.

## Halt-and-Report Conditions

These are the **only** reasons a run stops early. Everything else — including conformance drift — defers to the run report.

1. **Retry exhausted** — a story fails all 3 attempts.
...
3. **Significant breaking contract change** — a shared-interface break surfaced during implementation whose blast radius **cascades beyond the current story** (forces a shared-spec revision or changes across consuming repos). **Halt and escalate to the user for review.**
...
- Deterministic names: branch `mexec/<plan-id>/<story-id>/<attempt>`; worktree path likewise unique per attempt.
...
After the last wave, invoke **`/mverify`** across every repo the plan touched (its step-6 sweep). Record the result in `state.yaml`'s `conformance` block and the run report. **Drift does not halt the run** — a gatekept manual run leaves the drift for the human to act on next (`mspec`/`mreverse`); an autonomous `/mquick` run folds it into its escalation report.
...
`mexecute` runs to completion without between-wave gates. It interacts with the user **only** to escalate a **significant breaking contract change** (halt condition 3) — present the break, its blast radius, and the options (revise the shared spec / change consuming repos / abort), and wait for the decision before resuming.
